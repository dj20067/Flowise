                            ...
                            <Box sx={{ borderRadius: 2, border: 1, borderColor: theme.palette.grey[900] + 25, overflow: 'hidden' }}>
                                <Table aria-label='chatflow table'>
                                    <TableHead sx={{ height: 48 }}>
                                        <TableRow>
                                            <StyledTableCell sx={{ width: '30%' }}>{t('apikey.table.chatflowName')}</StyledTableCell>
                                            <StyledTableCell sx={{ width: '20%' }}>{t('apikey.table.modifiedOn')}</StyledTableCell>
                                            <StyledTableCell sx={{ width: '50%' }}>{t('apikey.table.category')}</StyledTableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
...
const APIKey = () => {
    ...
    const addNew = () => {
        const dialogProp = {
            title: t('apikey.dialog.add.title'),
            type: 'ADD',
    ...
    const edit = (key) => {
        const dialogProp = {
            title: t('apikey.dialog.edit.title'),
            type: 'EDIT',
            cancelButtonName: t('cancel'),
    ...
    const uploadDialog = () => {
        const dialogProp = {
            type: 'ADD',
            cancelButtonName: t('cancel'),
            confirmButtonName: t('apikey.actions.upload'),
            data: {}
        }
    ...
    const deleteKey = async (key) => {
        const confirmPayload = {
            title: t('apikey.confirm.delete.title'),
            description:
                key.chatFlows.length === 0
                    ? t('apikey.confirm.delete.descriptionNoUsage', { name: key.keyName })
                    : t('apikey.confirm.delete.descriptionWithUsage', { name: key.keyName, count: key.chatFlows.length }),
            confirmButtonName: t('chatflows.actions.delete'),
            cancelButtonName: t('cancel'),
        ...
        if (isConfirmed) {
            try {
                ...
                if (deleteResp.data) {
                    enqueueSnackbar({
                        message: t('apikey.snackbar.deleteSuccess'),
                        options: {
                ...
                enqueueSnackbar({
                    message: `${t('apikey.snackbar.deleteErrorPrefix')}${
                ...
                {error ? (
                    ...
                    <Stack flexDirection='column' sx={{ gap: 3 }}>
                        <ViewHeader
                            ...
                            search={true}
                            searchPlaceholder={t('apikey.view.searchPlaceholder')}
                            title={t('apikey.view.title')}
                            description={t('apikey.view.description')}
                        >
                            <PermissionButton
                            ...
                            <>
                                ...
                                >
                                    <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                                        ...
                                        >
                                            <TableRow>
                                                <StyledTableCell>{t('apikey.table.keyName')}</StyledTableCell>
                                                <StyledTableCell>{t('apikey.table.apiKey')}</StyledTableCell>
                                                <StyledTableCell>{t('apikey.table.usage')}</StyledTableCell>
                                                <StyledTableCell>{t('apikey.table.updated')}</StyledTableCell>
                                                <Available permission={'apikeys:update,apikeys:create'}>
