            ...
            <DialogContent>
                <Box sx={{ p: 2 }}>
                    <Stack sx={{ position: 'relative' }} direction='row'>
                        <Typography variant='overline'>
                            {t('assistants.openai.dialogs.load.fields.credential')}
                    ...
                    <CredentialInputHandler
                        ...
                        inputParam={{
                            label: t('assistants.openai.dialogs.load.fields.connectCredential'),
                            name: 'credential',
                            type: 'credential',
                ...
                {credentialId && (
                    <Box sx={{ p: 2 }}>
                        <Stack sx={{ position: 'relative' }} direction='row'>
                            <Typography variant='overline'>
                                {t('assistants.openai.dialogs.load.fields.assistants')}
                                <span style={{ color: 'red' }}>&nbsp;*</span>
                            </Typography>
                        </Stack>
                        <Dropdown
                            ...
                            value={selectedOpenAIAssistantId ?? t('credentials.common.chooseOption')}
                        />
                    </Box>
                )}
            </DialogContent>
            {selectedOpenAIAssistantId && (
                <DialogActions>
                    <StyledButton variant='contained' onClick={() => onAssistantSelected(selectedOpenAIAssistantId, credentialId)}>
                        {t('assistants.openai.dialogs.load.confirm')}
                    </StyledButton>
                </DialogActions>
            )}
        </Dialog>
